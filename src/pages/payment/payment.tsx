...
export const SellerPayment = () => {
  const { getAllPayment } = useActions();
  ...
  const { payment, loading } = useTypedSelector((state) => state.payment);

  const sortPayment = payment?.filter<IPayment>((item) => item.user_id.id == user.id);
                ...
                <S.RcTable>
                  <S.StatWrapper>
                    <S.RcTableContent>
                        ...
                        <S.Tbody>
                          ...
                          {loading.getAll ? (
                            <S.Tr data-row-key="0.5862388099375022">
                              <S.Td>
                                <Skeleton isLoaded={!loading.getAll}>
                                  0
                                </Skeleton>
                              </S.Td>
                              <S.Td>
                            ...
                            sortPayment?.map((item: IPayment) => (
                              <S.Tr data-row-key="0.5862388099375022">
                                <S.Td>
                                  {item?.created_at?.substring(0,10)}
                                </S.Td>
                                <S.Td>{item?.payment_card}</S.Td>
                                <S.Td>{item?.payment_amount}</S.Td>
                                <S.Td>{item?.payment_status}</S.Td>
                              </S.Tr>
                            ))
                          ) : (
                            <S.NoDataTable>
                      ...
                      <S.Td>
                        <strong style={{ display: "flex", gap: "15px" }}>
                          JAMI:{" "}
                          <Skeleton isLoaded={!loading.getAll}>
                            {sortPayment?.length || 0} ta
                          </Skeleton>{" "}
                        </strong>
                      </S.Td>
                    </S.RcTableContent>
